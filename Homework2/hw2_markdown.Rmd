...
This can be explained by Rpl14 being greatly conserved throughout species, and that this particular sequence at some point has been replicated and inserted randomly in the genome. Thus, the postulated 'new gene' is actually a pseudogene (segment of DNA that is related to a real gene). Since it is highly unlikely that the final processed transcript will be inserted in association with a promoter, it will never be transcribed but it will still resemble the sequence of the original gene due to high conservation (as mentioned earlier).
